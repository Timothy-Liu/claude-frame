...
export class ChatPanel {
  ...
  private async listSessionsWithTitles(projectId: string): Promise<Session[]> {
    ...
    return sessions.map((s) => {
      ...
      // Map the high-fidelity color hexes/names to corresponding visual emojis!
      const emojiMap: Record<string, string> = {
        '#e74c3c': '🔴', '#ec407a': '🔴', 'red': '🔴',
        '#007acc': '🔵', '#2f5778': '🔵', 'blue': '🔵',
        '#28a745': '🟢', '#16a085': '🟢', '#7cb342': '🟢', 'green': '🟢',
        '#9b59b6': '🟣', '#5e35b1': '🟣', 'purple': '🟣',
        '#f1c40f': '🟡', '#fb8c00': '🟡', 'yellow': '🟡',
        '#7f8c8d': '⚪'
      };
      const emoji = emojiMap[color] || '🔵';

      const shortId = s.id.slice(0, 8);
      const displayTitle = meta?.title && meta.title.trim() ? meta.title : `Session ${shortId}`;

      return { ...s, title: `${emoji} ${displayTitle}` };
    });
  }
